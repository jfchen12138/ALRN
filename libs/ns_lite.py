...
class FourierTransformer2DLite(nn.Module):
    def __init__(self, **kwargs):
        ...
        self._get_setting()
        self._initialize()
        self.__name__ = self.attention_type.capitalize() + 'Transformer2+1D'
        
        if self.attention_type == "Lrk":
            self.v = Parameter(0.01*torch.randn(self.k))
            self.s = Parameter(0.1*torch.rand(self.seq_len, self.k))
            self.d = Parameter(0.1*torch.rand(self.k, self.seq_len))
    ...
    def forward(self, node, edge, pos, grid=None):
        ...
        for encoder in self.encoder_layers:
            if self.return_attn_weight:
                if self.attention_type != "Lrk":
                    x, attn_weight = encoder(x, pos)
                    attn_weights.append(attn_weight)
                    
                else:
                    x, attn_weight = encoder(x, pos, s = self.s, v = v, d= self.d)
                    attn_weights.append(attn_weight)
            else:
                if self.attention_type != "Lrk":
                    x = encoder(x, pos)
                    ...
                    x = encoder(x, pos, s = self.s, v = v, d= self.d)
...
def train_batch_ns(model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.99, L1 = False,thred=0 ,alpha=0, beta=0):
    optimizer.zero_grad()
    ...
    if L1:
        ...
        norm = beta*torch.sum(torch.abs(model.v)**2).item()
    loss1 = loss_total/steps      
    ...
    return (loss_total.item(), loss1.item(), error.item()/steps, reg_total/steps, norm), u_preds, None

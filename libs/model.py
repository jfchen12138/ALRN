...
# class FourierTransformer3D(nn.Module):
#     def __init__(self, **kwargs):
#         super(FourierTransformer3D, self).__init__()
...
#             self.v = Parameter(0.1*torch.randn(self.k))
#             self.s = Parameter(torch.rand(self.seq_len, self.k))
#             self.d = Parameter(torch.rand(self.k, self.seq_len))
...
#         node = torch.cat([node.view(bsz, -1, 1), pos],
#                          dim=-1)
...
#                     x, attn_weight= encoder(x, pos, weight,s = self.s, v = torch.abs(self.v)**2, d= self.d)
#                     attn_weights.append(attn_weight)
                
#             elif self.attention_type != 'official':
#                 x = encoder(x, pos, weight,s = self.s, v = torch.abs(self.v)**2, d= self.d)
...
#         self.feat_extract = Identity(in_features=self.node_feats+self.pos_dim,
#                                      out_features=self.n_hidden)

#     def _get_scaler(self):
#         if self.downscaler_size:
#             self.downscaler = DownScaler3D(in_dim=self.node_feats,
...
#             self.upscaler = UpScaler3D(in_dim=self.n_hidden,
#                                      out_dim=self.n_hidden,
...
#             self.regressor = PointwiseRegressor3D(in_dim=self.n_hidden,
...
class SimpleTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model=96,
        ...
        if attention_type == "Lrk":
            self.attn = LrkAttention(n_head = n_head,
                                     d_model=d_model,
                                     attention_type=attention_type,
                                     pos_dim=pos_dim
                                    )
        if attention_type in ["fourier", "galerkin"]:
    ...
    def forward(self, x, pos=None, weight=None, s = None, v = None, d= None):
        '''
        ...
        if pos is not None and self.pos_dim > 0:
            if self.attention_type != "Lrk":
                ...
                att_output, attn_weight= self.attn(
                    x, x, x, pos=pos, weight=weight, s = s, v = v, d= d)  # encoder no mask
            
        else:
            if self.attention_type != "Lrk":
                att_output, attn_weight= self.attn(x, x, x, weight=weight)
            else:
                att_output, attn_weight= self.attn(x, x, x, weight=weight, s = s, v = v, d= d)
...
# class PointwiseRegressor3D(nn.Module):
#     def __init__(self, in_dim,  # input dimension
#                  n_hidden,
#                  out_dim,  # number of target dim
#                  num_layers: int = 2,
...
#                  return_latent=False,
#                  debug=False):
#         super(PointwiseRegressor3D, self).__init__()
...
#                  interp_size=None,
#                  debug=False):
#         super(DownScaler3D, self).__init__()
        
#         if downsample_mode == 'interp':
...
#         '''
#         3D:
#             Input: (-1, n, n, n, in_dim)
#             Output: (-1, n_s, n_s, n_s, out_dim)
#         '''
#         n_grid = x.size(1)
#         bsz = x.size(0)
#         x = x.view(bsz, n_grid, n_grid, n_grid, self.in_dim)
...
#                  interp_size=None,
#                  debug=False):
#         super(UpScaler3D, self).__init__()
        
       
#         hidden_dim = default(hidden_dim, in_dim)
     
#         if upsample_mode == 'interp':
...
class SimpleTransformer(nn.Module):
        ...
        for encoder in self.encoder_layers:
            if self.return_attn_weight:
                if self.attention_type != "Lrk":
                    ...
                    x, attn_weight= encoder(x, pos, weight,s = self.s, v = v, d= self.d)
                    attn_weights.append(attn_weight)
                ...
                if self.attention_type != "Lrk":
                    ...
                    x = encoder(x, pos, weight, s = self.s, v = v, d= self.d)
    ...
    def _get_setting(self):
        ...
        self.attention_types = ['fourier', 'integral',
                                'cosine', 'galerkin', 'linear', 'softmax', "Lrk"]
...
class FourierTransformer2D(nn.Module):
    def __init__(self, **kwargs):
        ...
        if self.attention_type == "Lrk":
            self.v = Parameter(0.1*torch.randn(self.k))
            self.s = Parameter(0.01*torch.rand(self.seq_len, self.k))
            self.d = Parameter(0.01*torch.rand(self.k, self.seq_len))
...
class ResNet(torch.nn.Module):
    def __init__(self, input_width, layer_width):
        ...
        self.linear_out = torch.nn.Linear(layer_width, 2)

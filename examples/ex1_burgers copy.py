...
#     scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.9)
    ...
    result = run_train(model, loss_func, metric_func,
                       ...
                       L1 = False,
                       beta = beta)
    train_loss = np.concatenate((train_loss, result["loss_train"]))
    valid_loss = np.concatenate((valid_loss, result["loss_val"]))
    
    fig, axs = plt.subplots(1, 2, figsize=(10, 5))  # 1行2列的子图

    ind = np.arange(200)
    axs[0].plot(ind, train_loss, label='sin(x)', color='blue')

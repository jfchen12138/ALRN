...
#         grad_x = (u[:, d:, s:-s, s:-s] - u[:, :-d, s:-s, s:-s])/d
#         grad_y = (u[:, s:-s, d:, s:-s] - u[:, s:-s, :-d, s:-s])/d
#         grad_z = (u[:, s:-s, s:-s, d:] - u[:, s:-s,s:-s,  :-d])/d
...
#         self.n_grid_fine = 33 # finest resolution along x-, y- ，z-dim
#         self.subsample_method = subsample_method  # 'interp' or 'nearest'
#         # sampling resolution for nodes subsampling along x-, y-. z-
...
#         self.pos= self.get_grid(self.n_grid)
...
#         self.node_features = nodes  # (N, n, n, n, 1)
#         self.target = targets  # (N, n, n, n, 1)
#         self.target_grad = targets_grad  # (N, n, n, n, 3)
...
#         targets = u  # (N, 51, 51, 51)
#         targets_gradx, targets_grady, targets_gradz = self.central_diff(
#             targets, self.h)  # (N, n_f, n_f)
#         targets_gradx = targets_gradx[:, ::s, ::s, ::s]
#         targets_grady = targets_grady[:, ::s, ::s, ::s]
#         targets_gradz = targets_gradz[:, ::s, ::s, ::s]
...
#             [targets_gradx, targets_grady, targets_gradz], axis=-1)  # (N, n, n, n, 3)

#         targets = targets[:, ::s, ::s, ::s].reshape(batch_size, n, n, n, 1)
#         nodes = a[:, ::s, ::s, ::s].reshape(batch_size, n, n , n, 1)
...
#         # b = x.shape[0]
#         if padding:
#             x = np.pad(x, ((0, 0), (1, 1), (1, 1),(1, 1)),
#                        'constant', constant_values=0)
#         d, s= 2, 1  # dilation and stride
#         grad_x = (x[:, d:, s:-s, s:-s] - x[:, :-d, s:-s, s:-s])/d  
#         grad_y = (x[:, s:-s, d:, s:-s] - x[:, s:-s, :-d, s:-s])/d  
#         grad_z = (x[:, s:-s, s:-s, d:] - x[:, s:-s ,s:-s, :-d])/d

#         return grad_x/h, grad_y/h, grad_z/h
...
#         if return_elem:
#             grid = np.c_[x.ravel(), y.ravel(), z.ravel()]  # (n_node, 3)
#             return grid
#         else:
#             if return_boundary:
#                 x = x[::s, ::s, ::s]
...
#         if last_digit < 5:
#             factor += 5e-3
#         factor = int(factor/5e-3 + 5e-1 ) * 5e-3+1e-2     
#         down_factor = (factor, factor)           
#         n_m = round(n_f*factor)-1
#         up_size = ((n_m, n_m,n_m), (n_f, n_f, n_f))
#         down_size = ((n_m, n_m, n_m), (n_c, n_c, n_c))
#         if scale_factor:

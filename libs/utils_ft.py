...
from torch.optim.lr_scheduler import OneCycleLR, _LRScheduler, StepLR
...
# def train_batch_darcy_3d(model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.99, attention_type = "fourier",thred=0 ,alpha=0, beta=0):
#     optimizer.zero_grad()
...
# def validate_epoch_darcy_3d(model, metric_func, valid_loader, device):
#     model.eval()
#     metric_val = []
...
def get_args_1d():
    ...
    parser.add_argument('--subsample', type=int, default=2, metavar='subsample',
...
###subsample_nodes=3,subsample_attn=10,
def get_args_2d(subsample_nodes=3,
    ...
    parser.add_argument('--no-scale-factor', action='store_true', default=True,
                        help='use size instead of scale factor in interpolation') ##default False
    parser.add_argument('--no-cuda', action='store_true', default=False,
                        help='disables CUDA training')
    parser.add_argument('--show-batch', action='store_true', default=False,
                        help='show batch training result')
...
def get_args_3d(subsample_nodes=1,
                subsample_attn=1,
                gamma=0.5,
                noise=0.0,
                ffn_dropout=0.1,
    ...
    parser = argparse.ArgumentParser(description='Example 4: Darcy 3D interface flow')

    n_grid = int(((33 - 1)/subsample_nodes) + 1)
    n_grid_c = int(((33 - 1)/subsample_attn) + 1)

    parser.add_argument('--subsample-nodes', type=int, default=subsample_nodes, metavar='subsample',
                        help=f'input fine grid sampling from 51x51 (default: {subsample_nodes} i.e., {n_grid}x{n_grid} grid)')
    parser.add_argument('--subsample-attn', type=int, default=1, metavar='subsample_attn',
                        help=f'input coarse grid sampling from 10x10 (default: {subsample_attn} i.e., {n_grid_c}x{n_grid_c} grid)')
    parser.add_argument('--batch-size', type=int, default=4, metavar='bsz',
                        help='input batch size for training (default: 4)')
    parser.add_argument('--val-batch-size', type=int, default=4, metavar='bsz',
                        help='input batch size for validation (default: 4)')
...
def train_batch_burgers(model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.999, L1=False, thred=0 ,alpha=0, beta=0):
    optimizer.zero_grad()
    x, edge = data["node"].to(device), data["edge"].to(device)
    pos, grid = data['pos'].to(device), data['grid'].to(device)
    ...
    if out.size(2) == 2:
        ...
        u_pred = out[..., 0]
        loss1, reg, ortho, _ = loss_func(
            u_pred, u, targets_prime=up, preds_latent=y_latent)
  
    norm = torch.zeros(1).cuda()  ##\|V\|_L1

    if L1:
        norm = beta*torch.sum(torch.abs(model.v)**2)
        

    loss = loss1 + reg + ortho+ norm
...
def matrix_F_norm(A):
    if A.ndim==2:
        return torch.sqrt(torch.sum(A**2, dim=[0,1]))
    if A.ndim==3:
        return torch.sqrt(torch.sum(A**2, dim=[0,1,2]))
    
def train_batch_darcy(model, loss_func, data, optimizer, lr_scheduler, device, grad_clip=0.99, L1 = False, beta=0):
    optimizer.zero_grad()
    a, x, edge = data["coeff"].to(device), data["node"].to(
        device), data["edge"].to(device)
    pos, grid = data['pos'].to(device), data['grid'].to(device)
...
#         print(out.shape)
    ...
    if out.ndim == 4:
        u_pred, pred_grad, target = out[..., 0], out[..., 1:], u[..., 0]
        loss1, reg, _, _ = loss_func(u_pred, target, pred_grad, gradu, K=a)
    elif out.ndim == 3:
        u_pred, u = out[..., 0], u[..., 0]
        loss1, reg, _, _ = loss_func(u_pred, u, targets_prime=gradu, K=a)
    ...
    return (loss.item(),(loss1+reg).item(), reg.item(), norm.item()), u_pred, up_pred
            ...
            with tqdm(total=len(train_loader), disable=tqdm_epoch) as pbar_batch:
                for batch in train_loader:
                    if is_epoch_scheduler:
                        loss, _, _= train_batch(model, loss_func, batch, optimizer,None, device, grad_clip=grad_clip, 
                                                L1 = L1, beta = beta)
                    else: 
                        
                        loss, _, _ = train_batch(model, loss_func, batch, optimizer,lr_scheduler, device, grad_clip=grad_clip, 
                                                 L1 = L1, beta = beta)
                    ...
                    pbar_batch.update()
#             print(f"epoch {epoch} : {_loss_mean[1]}")
            loss_train.append(_loss_mean)
            sparse_temp = loss_epoch[-1][-1]
...
#                 elif attention_type == "Lrk" and val_metric == best_val_metric and sparse_temp < best_sparse :
#                     best_val_epoch = epoch
#                     best_val_metric = val_metric
#                     best_sparse = sparse_temp
#                     stop_counter = 0
#                     torch.save(model.state_dict(), os.path.join(
#                         model_save_path, model_name))
